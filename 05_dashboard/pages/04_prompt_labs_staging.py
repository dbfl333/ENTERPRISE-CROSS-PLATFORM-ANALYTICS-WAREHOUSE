...
st.set_page_config(page_title="Prompt Labs Staging - Analytics Warehouse", layout="wide")

st.title("🤖 Tenant C: Agentic Prompt Labs")
st.markdown("Staging viewport for AI Agent telemetry logs, token tracking, and service latencies.")

# Awaiting Launch Alert Banner
st.warning("🟠 **Project Status: Pre-Launch / Awaiting Day 1 Stream**  \nNo live user data is being generated yet. No synthetic/fake traffic is injected to ensure reporting hygiene.")
...
st.markdown("This schema has been provisioned inside `analytics_production.duckdb` and is ready for real-time telemetry stream ingestion on Day 1 launch:")
...
st.markdown("The landing CSV file `02_raw_data/prompt_telemetry_staging.csv` is correctly positioned to receive write logs from the production application:")
